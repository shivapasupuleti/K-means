...
def bisecting_kmeans(data, max_clusters):
    ...
    Returns:
        dict: Dictionary containing cluster assignments for each number of clusters.
    """
    cluster_assignments = {}
    remaining_clusters = [np.ones(data.shape[0], dtype=bool)]  # Start with a single cluster containing all data points

    for k in range(2, max_clusters + 1):
        # Choose the cluster to split
        cluster_to_split_idx = np.random.choice(len(remaining_clusters))
        cluster_to_split = remaining_clusters[cluster_to_split_idx]

        # Perform k-means clustering with k=2 on the chosen cluster
        centroids, assignments = kmeans(data[cluster_to_split], 2)

        # Split the chosen cluster into two
        split_cluster_1 = cluster_to_split.copy()
        split_cluster_1[cluster_to_split] = assignments == 0
        split_cluster_2 = cluster_to_split.copy()
        split_cluster_2[cluster_to_split] = assignments == 1

        # Update remaining clusters
        remaining_clusters.pop(cluster_to_split_idx)
        remaining_clusters.append(split_cluster_1)
        remaining_clusters.append(split_cluster_2)

        # Update cluster assignments for the current number of clusters
        cluster_assignments[k] = np.full(data.shape[0], -1)
        for i, cluster in enumerate(remaining_clusters):
            cluster_assignments[k][cluster] = i

    return cluster_assignments

def plot_silhouette_KmeansBisecting(data):
    
    # Plot the Silhouette coefficient for K=1-9 numbers of clusters using Bisecting k-Means.

    cluster_assignments = bisecting_kmeans(data, 9)
    silhouette_coefficients = []
    ...
    for k in range(1, 10):
        if k == 1:
            ...
            centroids = initialize_centroids_plusplus(data, k)
            silhouette_coefficient = compute_silhouette_coefficient(data, cluster_assignments[k], centroids)
        silhouette_coefficients.append(silhouette_coefficient)

    plt.figure(figsize=(8, 6))
    plt.plot(range(1, 10), silhouette_coefficients, marker='o')
    plt.xlabel('Clusters(k=1-9)')
    plt.ylabel('Silhouette Coefficient')
    plt.title('Silhouette Coefficient vs Clusters(k=1-9)(Bisecting k-Means)')
    plt.show()

data, labels = load_dataset('dataset')
plot_silhouette_KmeansBisecting(data)